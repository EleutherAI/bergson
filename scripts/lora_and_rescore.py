#!/usr/bin/env python3
"""
SFT OLMo-3-7B-Instruct on SmolLM2-135M-10B, then re-run trackstar scoring
against MMLU sociology to compare with the base model.

Results are saved to runs/olmo3_7b_sft_mmlu_sociology_smollm2_10b/.
"""
...
class ExperimentConfig:
    ...
    # SFT
    output_dir: str = "runs/olmo3_7b_sft_model"
    ...
    sft_run: str = "runs/olmo3_7b_sft_mmlu_sociology_smollm2_10b"
    token_batch_size: int = 2048
    projection_dim: int = 16
    nproc_per_node: int = 8


def train_sft(cfg: ExperimentConfig):
    """Launch SFT training as a subprocess to ensure GPU memory is freed after."""
    print("=" * 60)
    print("Step 1: SFT on SmolLM2-135M-10B (10k docs)")
    print("=" * 60)

    output_path = Path(cfg.output_dir)
    if (output_path / "adapter_config.json").exists():
        print(f"  SFT model already exists at {output_path}, skipping training.")
        return

    # Run SFT in a subprocess so GPU memory is fully freed when it exits
    cmd = [
        sys.executable,
        __file__,
        "--sft-only",
...
def _run_sft_worker(args):
    """Actual SFT training logic, called in a subprocess."""
    import torch
...
def run_trackstar(cfg: ExperimentConfig):
    ...
    print("=" * 60)
    print("Step 2: Trackstar scoring with SFT model")
    print("=" * 60)
...
def main():
    ...
    # Step 2: Trackstar with SFT model
    run_trackstar(cfg)
...
if __name__ == "__main__":
    if "--sft-only" in sys.argv:
        ...
        _run_sft_worker(parser.parse_args())
    else:
        main()
