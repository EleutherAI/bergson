...
Uses fp32, no FSDP, and times each trackstar step.
"""
...
class ExperimentConfig:
    ...
    # Trackstar scoring
    run_path: str = "runs/olmo3_7b_base_trackstar_timed"
...
def run_trackstar(exp_cfg: ExperimentConfig):
    ...
    print("=" * 60)
    print("Trackstar scoring with base model (fp32, no FSDP)")
    print("=" * 60)

    run_path = Path(exp_cfg.run_path)

    # Build the bergson trackstar command — one step at a time so we can
    # measure wall-clock duration per step.  The --resume flag lets us
    # re-enter safely if a step was already completed.
    base_cmd = [
        "bergson",
        "trackstar",
        str(run_path),
    ...
    # Run the full pipeline in one go but wrap it so we can parse step
    # timings from output.  Trackstar prints "Step N/5: ..." before each
    # step, so we capture that to measure durations.
