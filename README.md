...
`bergson trackstar`, `bergson ekfac`, and `bergson trak` all orchestrate multi-step attribution recipes over a model checkpoint. `bergson tracin` sums gradient dot products over the checkpoints of an SGD run. `bergson trak` also supports [ensembling](https://arxiv.org/abs/2303.14186) over independently trained models.
