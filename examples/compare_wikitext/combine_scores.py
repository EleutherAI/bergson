"""Linear combination of score stores, e.g. debiased scores.
    ...
    python examples/compare_wikitext/combine_scores.py \\
        --term 1 runs/compare_wikitext/dda/finetuned/scores \\
        --term -1 runs/compare_wikitext/dda/base/scores \\
        --out runs/compare_wikitext/dda/scores
...
def main():
    ...
    for name in first.dtype.names:
        if name.startswith("score_"):
            combined[name] = sum(
                c * s[name].astype(np.float64) for c, s in zip(coefs, stores)
            )
