...
class Score_Trajectory:
    ...
    window: int = 0
    """If > 1, also overlay the window-N step-normalisation curve and the
    residual post-normalisation level."""

    out: str = ""
    """Output PNG path (default: ``<run_path>/score_vs_step.png``)."""

    def execute(self):
        path = plot_score_trajectory(
            self.run_path, window=self.window, out=self.out or None
        )
        print(f"wrote {path}")
