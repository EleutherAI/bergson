...
class HookCollectorBase(ContextDecorator, ABC):
    ...
    def collection_mask(self, module: nn.Module) -> Tensor | None:
        """The mask over ``module``'s gradient-carrying positions.

        An expanded MoE expert sees one row per token routed to it rather than
        the batch's ``[N, S]`` positions, so its mask is built from the
        ``_positions`` grid ``bergson.moe.moe_forward`` leaves behind.
        """
    ...
    def __enter__(self):
        ...
        if self.attribute_tokens and any(
            ...
            raise ValueError(
                ...
                "not line up with the batch's token positions."
            )
