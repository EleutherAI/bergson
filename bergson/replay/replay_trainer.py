...
class ReplayTrainer:
    ...
    Important attributes:
        ...
        - **place_model_on_device** -- Whether or not to automatically place the model on the device - it will be set
          to `False` if model parallel is used, or if the default
    ...
    def __init__(
        ...
        train_dataset: (
            Union[Dataset, IterableDataset, "datasets.Dataset"] | None
        ) = None,
        eval_dataset: (
            Union[Dataset, dict[str, Dataset], "datasets.Dataset"] | None
        ) = None,
        processing_class: (
        ...
        if (
            args.fp16
            and args.device == torch.device("cpu")
            and not is_torch_greater_or_equal_than_2_3
        ):
            raise ValueError(
                "Tried to use `fp16` but it is not supported on cpu. You need to have torch>=2.3"
            )
        ...
        if getattr(self.model, "config", None) is not None:
            # TODO Lucia understand what the sensible default is here
            self.model.config.use_cache = (
                self.args.use_cache if hasattr(self.args, "use_cache") else False
            )
    ...
    def _get_dataloader(
        ...
        should_fork = (
            torch.backends.mps.is_available() and self.args.dataloader_num_workers > 1
        )
    ...
    def _save_checkpoint(self, model, trial):
        # In all cases, including ddp/dp, self.model is always a reference to the model we
    ...
    def _prepare_context_parallel_inputs(
        self, model, inputs: dict[str, torch.Tensor | Any]
    ):
        """
    ...
    def floating_point_ops(self, inputs: dict[str, torch.Tensor | Any]):
        ...
        ) in inputs and hasattr(self.model, "num_parameters"):
            return (
                6
                * inputs[main_input].numel()
                * self.model.num_parameters(exclude_embeddings=True)
            )
        return 0
    ...
    def _finish_current_push(self):
        ...
        if self.push_in_progress is not None and not self.push_in_progress.is_done():
            logger.info(
                "Waiting for the current checkpoint push to be finished, "
    ...
    def create_accelerator_and_postprocess(self):
        ...
        if "num_steps" in grad_acc_kwargs:
            if self.args.gradient_accumulation_steps > 1:
                ...
                raise ValueError(
                    ...
                    "If using the passed `AcceleratorConfig` is desired, do not set the "
        ...
        if non_blocking and not self.args.dataloader_pin_memory:
            logger.warning(
                "`non_blocking` is enabled but `dataloader_pin_memory` is not. For the "
                "best performance, it's recommended to enable both."
            )
        ...
        fsdp_plugin = None
        if (
            hasattr(self.args, "fsdp_plugin_args")
            and self.args.fsdp_plugin_args is not None
        ):
        ...
        if self.args.parallelism_config is not None:
            ...
            if not is_accelerate_available(min_accelerate_version):
                raise ImportError(
                    f"ParallelismConfig requires accelerate>={min_accelerate_version}). "
        ...
        if getattr(self.model, "tp_size", None) is not None and self.model.tp_size > 1:
            ...
            if self.args.parallelism_config is not None:
                if is_accelerate_available("1.10.1"):
                    ...
                    raise ValueError(
                        "Requires accelerate>1.10.1 to use Tensor Parallelism."
                    )
        ...
        # `save_only_model` can't be used with FSDP along with `load_best_model_at_end`
        if (
            ...
            raise ValueError(
                "FSDP can't be used with `save_only_model` along with `load_best_model_at_end`."
            )
    ...
    def set_initial_training_values(
        ...
        if len_dataloader is not None:
            ...
            # Case 3: We have a length but are using epochs, we can extrapolate the number
            #  of steps
            if epoch_based:
                max_steps = math.ceil(
