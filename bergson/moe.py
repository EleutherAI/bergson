"""Expose an MoE layer's fused experts as one module per expert projection.

In ``transformers`` 5.x an MoE layer holds every expert in a single 3D
``nn.Parameter`` instead of an ``nn.Linear`` each, so gradient collection skips
it and only attention and ``lm_head`` are attributed. Name such layers in
``ModelConfig.moe_experts`` and :func:`expand_moe` gives each expert projection
its own :class:`ExpertLinear` and replaces the layer's forward with a loop over
experts, which is the layout bergson already tracks.

The loop gathers each expert's routed tokens into ``[N, width, ...]``, one row
per example padded to the widest example, so the collectors see their usual
leading example dimension. Padding rows are zero and the ``_positions`` grid
each expert carries tells the collector which rows are real.
"""

import types
from fnmatch import fnmatchcase

import torch
import torch.nn as nn
from torch import Tensor

# The experts module only ever sees hidden states flattened to [N*S, hidden], so
# a pre-hook on its parent leaves the batch size here for the forward to read.
NUM_EXAMPLES_ATTR = "_bergson_num_examples"


class ExpertLinear(nn.Module):
    """Present one expert's slice of a fused MoE parameter as a linear layer.

    ``weight`` and ``bias`` are unregistered views, so the fused parameter stays
    the only copy of the weights and ``state_dict`` is unchanged. ``weight``
    keeps the parent's orientation, which ``LayerAdapter.weight_transposed``
    reports.
    """

    def __init__(self, experts: nn.Module, weight_name: str, expert_idx: int):
        super().__init__()

        # Via __dict__, because registering the parent as a child would make
        # named_modules() recurse back into it.
        self.__dict__["_experts"] = experts
        self.weight_name = weight_name
        self.expert_idx = expert_idx
        self.transposed = bool(experts.is_transposed)  # type: ignore[attr-defined]

        rows, cols = self.weight.shape
        self.in_features, self.out_features = (
            (rows, cols) if self.transposed else (cols, rows)
        )

    @property
    def weight(self) -> Tensor:
        return getattr(self._experts, self.weight_name)[self.expert_idx]

    @property
    def bias(self) -> Tensor | None:
        bias = getattr(self._experts, f"{self.weight_name}_bias", None)
        return None if bias is None else bias[self.expert_idx]

    def forward(self, x: Tensor) -> Tensor:
        out = x @ self.weight if self.transposed else x @ self.weight.mT
        bias = self.bias
        return out if bias is None else out + bias


def projection_names(experts: nn.Module) -> tuple[str, str]:
    """Name the two fused parameters holding each expert's projections."""
    return ("gate_up_proj" if experts.has_gate else "up_proj", "down_proj")  # type: ignore[attr-defined]


def moe_forward(
    self: nn.Module,
    hidden_states: Tensor,
    top_k_index: Tensor,
    top_k_weights: Tensor,
) -> Tensor:
    """Run each expert over its routed tokens, one ExpertLinear call at a time.

    Matches the fused implementations in ``transformers``, except that each
    expert's tokens are gathered into a per-example grid first. An expert with
    no tokens still runs, or its backward hook never fires and the index comes
    up a module short.
    """
    assert hidden_states.ndim == 2, f"Expected [N*S, hidden], got {hidden_states.shape}"
    num_tokens, hidden_dim = hidden_states.shape
    num_examples = getattr(self, NUM_EXAMPLES_ATTR)
    assert (
        num_tokens % num_examples == 0
    ), f"{num_tokens} tokens do not divide into {num_examples} examples"
    seq_len = num_tokens // num_examples

    top_k_index = top_k_index.reshape(num_tokens, -1)
    top_k_weights = top_k_weights.reshape(num_tokens, -1)
    up_name, down_name = projection_names(self)
    out = torch.zeros_like(hidden_states)

    for expert_idx in range(self.num_experts):  # type: ignore[attr-defined]
        # Expert-parallel layouts route padding tokens to sentinel indices past
        # the last expert, so matching on the index alone already drops them.
        picked = top_k_index == expert_idx  # [N*S, top_k]
        weights = (top_k_weights * picked).sum(-1)  # [N*S]
        routed = picked.any(-1).view(num_examples, seq_len)  # [N, S]

        # One row per example, padded to the widest example's token count.
        # positions records where each row came from, -1 for the padding.
        example, pos = routed.nonzero(as_tuple=True)
        token = example * seq_len + pos
        col = (routed.cumsum(1) - 1)[example, pos]
        width = max(int(routed.sum(1).max()), 1)

        expert = getattr(self, f"expert_{expert_idx}")
        positions = pos.new_full((num_examples, width), -1)
        positions[example, col] = pos
        for projection in expert.children():
            projection._positions = positions

        a = hidden_states.new_zeros(num_examples, width, hidden_dim)
        a[example, col] = hidden_states[token]

        h = getattr(expert, up_name)(a)
        h = self._apply_gate(h) if self.has_gate else self.act_fn(h)  # type: ignore[attr-defined]
        h = getattr(expert, down_name)(h)

        h = h[example, col] * weights[token].unsqueeze(-1)
        out = out.index_add(0, token, h.to(out.dtype))

    return out


def _record_num_examples(experts: nn.Module):
    """Build a pre-hook that records its module's batch size on ``experts``."""

    def hook(module: nn.Module, args: tuple):
        assert args and args[0].ndim == 3, (
            f"{type(module).__name__} was expected to take [N, S, hidden] as "
            f"its first positional argument"
        )
        setattr(experts, NUM_EXAMPLES_ATTR, args[0].shape[0])

    return hook


def _check_fused(name: str, experts: nn.Module) -> None:
    """Fail on a pattern that matched something other than a fused MoE layer."""
    down = getattr(experts, "down_proj", None)
    attrs = ("num_experts", "has_gate", "is_transposed")
    if (
        not isinstance(down, nn.Parameter)
        or down.ndim != 3
        or not all(hasattr(experts, attr) for attr in attrs)
    ):
        raise ValueError(
            f"moe_experts matched '{name}' ({type(experts).__name__}), which is "
            f"not an MoE layer with fused expert parameters. Point the pattern "
            f"at the module holding the 3D gate_up_proj/down_proj parameters, "
            f"e.g. 'model.layers.*.mlp.experts'."
        )


def expand_moe(model: nn.Module, patterns: str) -> list[str]:
    """Give every expert projection of the matching MoE layers its own module.

    ``patterns`` is a comma-separated list of globs over ``named_modules()``.
    Returns the names of the per-expert modules, which are what gradient
    collection then picks up, whether this call added them or an earlier one did.
    """
    names = []
    matched = []

    for name, experts in list(model.named_modules()):
        if not any(fnmatchcase(name, p.strip()) for p in patterns.split(",")):
            continue

        matched.append(name)
        _check_fused(name, experts)

        projections = projection_names(experts)
        num_experts = int(experts.num_experts)  # type: ignore[arg-type]
        names += [
            f"{name}.expert_{i}.{w}" for i in range(num_experts) for w in projections
        ]
        if hasattr(experts, "expert_0"):
            continue

        for expert_idx in range(num_experts):
            expert = nn.Module()
            for weight_name in projections:
                expert.add_module(
                    weight_name, ExpertLinear(experts, weight_name, expert_idx)
                )
            experts.add_module(f"expert_{expert_idx}", expert)

        setattr(experts, NUM_EXAMPLES_ATTR, 1)
        experts.forward = types.MethodType(moe_forward, experts)
        parent = model.get_submodule(name.rpartition(".")[0])
        parent.register_forward_pre_hook(_record_num_examples(experts))

    if not matched:
        raise ValueError(
            f"moe_experts={patterns!r} matched no module. Available MoE-shaped "
            f"module names look like 'model.layers.0.mlp.experts'; print "
            f"[n for n, _ in model.named_modules()] to find yours."
        )
    return names
