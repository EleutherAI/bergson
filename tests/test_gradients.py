...
@pytest.mark.parametrize(
    "projection_dim,normalizer_kind,dtype",
    [
        (None, "adam", torch.float32),
        (4, "adam", torch.float32),
        (None, "adafactor", torch.float32),
        (4, "adam", torch.bfloat16),
        (4, "adafactor", torch.bfloat16),
    ],
)
...
# A split head's right projection matrix is named after the module when it has
# no bias, and after the head when it has one, so split heads are only checked
# without projection.
AUTOGRAD_CASES = [
    ...
    if not (split_heads and projection_dim)
]
    ...
    def expected(name, weight, bias, rows=slice(None)):
        ...
        if projection_dim:
            left, right = (
                ...
                for role, n in [("left", grad.shape[0]), ("right", grad.shape[1])]
            )
